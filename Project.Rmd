...
60/40 ratio was determined by a reading on stackexchange.com and other site.
...
values of classe are discrete, and as pointed out in the lectures, the Random Forest accuracy is high. We further try to improve the accuracy by use a 10 fold cross validation.  
...
We see that with k-fold cross-validation we achieve an accuracy of 0.99 or 99%.  This accuracy was achived when the number of variables per level (mtry) in the model was 27.
...
cmTrain <- confusionMatrix(hatTrain, trainCentScale[, classe])
cmTrain
```
...
that the OOB error rate is low, 0.84%, increasing our confidence in the model.
